$(function() {
	var barra = function() {
		var elemento = $('#gg2-barra-aux'),
			n_selecionados, n_itens, todos, itens_selecionados, texto_aux;
		function mostra() {
			elemento.removeClass('hide').show();
			texto_aux = (n_selecionados > 1) ? 'itens selecionados' : 'item selecionado';
			$('#gg2-barra-texto').html('Você tem ' + n_selecionados + ' ' + texto_aux);
			...
			if (typeof(ACAO_REMOVER) == 'string') {
				$('<button />')
					...
					.click(function() {
						...
						if (confirm('Deseja realmente remover (' + n_selecionados + ') ' + texto_aux + '?')) {
